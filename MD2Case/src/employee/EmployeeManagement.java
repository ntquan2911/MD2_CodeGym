...
public class EmployeeManagement {
    ...
    private static final String REAL_INCOME = "Lương thực lĩnh";
    ...
    private static final String EXAMPLE_FULL_NAME = "(VD:NguyenTienQuan)";
    private static final String EXAMPLE_AGE = "(VD:29)";
    private static final String EXAMPLE_ADDRESS = "(VD:HaNoi)";
...
//    }
    ...
    public void showInfo(Employee employee) {
        if (employee instanceof Developer) {
            System.out.printf(ID + ": %-5s, " + FULL_NAME + ": %-15s " + AGE + ": %-4s, " + ADDRESS + ": %-10s, " + PHONE_NUMBER + ": %-10s, " + REAL_INCOME + ":%-15s" + "QUẢN LÍ", employee.getId(), employee.getFullName(), employee.getAge(), employee.getAddress(), employee.getPhoneNumber(), employee.getRealIncome() + "(vnd)");
        } else if (employee instanceof Accountant) {
            System.out.printf(ID + ": %-5s, " + FULL_NAME + ": %-15s " + AGE + ": %-4s, " + ADDRESS + ": %-10s, " + PHONE_NUMBER + ": %-10s, " + REAL_INCOME + ":%-15s" + "KẾ TOÁN", employee.getId(), employee.getFullName(), employee.getAge(), employee.getAddress(), employee.getPhoneNumber(), employee.getRealIncome() + "(vnd)");
        } else if (employee instanceof Tester) {
            System.out.printf(ID + ": %-5s, " + FULL_NAME + ": %-15s " + AGE + ": %-4s, " + ADDRESS + ": %-10s, " + PHONE_NUMBER + ": %-10s, " + REAL_INCOME + ":%-15s" + "NHÂN VIÊN BÁN HÀNG", employee.getId(), employee.getFullName(), employee.getAge(), employee.getAddress(), employee.getPhoneNumber(), employee.getRealIncome() + "(vnd)");

        } else if (employee instanceof Security) {
            System.out.printf(ID + ": %-5s, " + FULL_NAME + ": %-15s " + AGE + ": %-4s, " + ADDRESS + ": %-10s, " + PHONE_NUMBER + ": %-10s, " + REAL_INCOME + ":%-15s" + "BẢO VỆ", employee.getId(), employee.getFullName(), employee.getAge(), employee.getAddress(), employee.getPhoneNumber(), employee.getRealIncome() + "(vnd)");
        }
    }
    ...
    public void sortSalary() {
        for (int i = 0; i < employeeList.size() - 1; i++) {
            for (int j = employeeList.size() - 1; j > i; j--) {
                if (employeeList.get(i).getRealIncome() < (employeeList.get(j).getRealIncome())) {
